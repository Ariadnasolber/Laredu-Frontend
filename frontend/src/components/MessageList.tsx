...
export default function MessageList() {
    ...
    return (
        <div className="mt-4 flex items-center justify-center flex-col">
            <h2 className="text-2xl font-bold mb-8 underline underline-offset-8 decoration-red-400">MENSAJES</h2>
            ...
            <form onSubmit={handleSendMessage} className="mb-4 flex space-x-2">
                ...
                <input
                    type="text"
                    placeholder="Mensaje"
                    className="border p-2 rounded w-100 bg-white"
                    value={content}
                    onChange={(e) => setContent(e.target.value)}
                />
                <button
                    type="submit"
                    className="bg-red-600 text-white p-2 rounded px-6"
                >
                    Enviar
                </button>
            </form>
            <ul className="space-y-2">
                {messages.map((msg) => (
                    <li
                        key={msg.id}
                        className="p-2 border rounded bgwhite shadow"
                    >
                        <strong>
                            {msg.sender_id === 2
                                ? "Yo"
